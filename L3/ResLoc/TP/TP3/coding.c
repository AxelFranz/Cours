...
{
  int count = 16;
  CodeWord_t res = *message << 8;
  while(count >= DEGREE_POLYNOM) {
    printf("%d\n",count);
    if(getNthBit(res,count) == 0) {
      count--; continue;
    }
    for(int i = 0; i <= DEGREE_POLYNOM; i++) {
      setNthBitCW(&res,count-i,getNthBit(res,count-i)^(POLYNOM[DEGREE_POLYNOM-i]));
    }
  }
  *message = xor(*message << 8,res);
  printBits(*message,"");
}
